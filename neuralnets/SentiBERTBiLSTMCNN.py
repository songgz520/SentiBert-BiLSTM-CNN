        ...
        for modelName in self.modelNames:
            ...
            for classifier in modelClassifier:
                ...
                if classifier == 'Softmax':
                    ...
                    x = Conv2D(16, (3, 3), activation='relu', padding='same', name='block1_conv1')(output)
                    x = Conv2D(8, (3, 3), activation='relu', padding='same', name='block1_conv2')(x)
                    x = MaxPooling2D((2, 2), strides=(2, 2), name='block1_pool')(x)
                    x = Conv2D(4, (3, 3), activation='relu', padding='same', name='block2_conv1')(x)
                    x = Conv2D(2, (3, 3), activation='relu', padding='same', name='block2_conv2')(x)
                    x = MaxPooling2D((2, 2), strides=(2, 2), name='block2_pool')(x)
                    output = Flatten()(x)
                    lossFct = 'sparse_categorical_crossentropy'            name=modelName + '_hidden_lin_layer')(output)
                  
                elif isinstance(classifier, (list, tuple)) and classifier[0] == 'LSTM':
                            
                    size = classifier[1]
                    if isinstance(self.params['dropout'], (list, tuple)): 
                        shared_layer = Bidirectional(LSTM(size, return_sequences=True, dropout=self.params['dropout'][0], recurrent_dropout=self.params['dropout'][1]), name=modelName+'_varLSTM_'+str(cnt))(output)
                    else:
                        """ Naive dropout """ 
                        shared_layer = Bidirectional(LSTM(size, return_sequences=True), name=modelName+'_LSTM_'+str(cnt))(output) 
                        if self.params['dropout'] > 0.0:
